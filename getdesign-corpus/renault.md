...
### Gradient System
- **Hero**: No decorative gradients — hero backgrounds are clean solids (pure black or pure white) that let vehicle photography and the yellow CTA do the chromatic work
- **PromoCard Overlay**: `linear-gradient(rgba(0,0,0,0.6) 0%, rgba(0,0,0,0) 40%)` — applied to card tops strictly as a legibility aid for heading text over photography
...
### Image Treatment
- **Hero**: Full-viewport carousel with clean solid backgrounds (pure black or pure white) and art-directed vehicle photography — edge-to-edge, no padding
...
- **PromoCard overlays**: `linear-gradient(rgba(0,0,0,0.6) → transparent)` creates depth within cards through transparency, used as a legibility tool rather than an aesthetic one
- **No blur effects** on UI elements — depth is communicated through shadow, photography, and the yellow-on-black contrast
...
- Use flat CSS gradients on UI surfaces — the only acceptable gradient is the top-of-card dark overlay used for text legibility over photography
...
- "Create a hero section with a solid Absolute Black (#000000) background, a centered vehicle image, a NouvelR Bold headline at 56px with 0.95 line-height in white, a Renault Yellow (#EFDF00) super-primary button 'Configure', and a Ghost button (transparent bg, white border, white text, 0px radius) 'Learn More'"
