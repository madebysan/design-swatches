...
**Key Characteristics:**
- Full-viewport hero sections (100vh) showing a single piece of car photography — no overlays, no tints, no decorative frames
- Pure white OR pure black as the canvas — never tinted, never gradient, never patterned
- Electric Blue (`#3E6AE1`) appears ONLY on primary CTA buttons and text links — never as atmospheric color, tint, glow, or fill
- Zero UI decoration anywhere on chrome: no shadows, no gradients, no borders on tiles, no patterns
- Universal Sans font family (Display + Text) unifying web, app, and in-car interfaces
- Headings are always neutral (`#171A20` on white, `#FFFFFF` on black) — no colored accents on type
- Photography carries 100% of the visual richness — the UI has no opinion of its own
- Transparent nav that floats directly over hero imagery, no shadow or border beneath
- 0.33s cubic-bezier transitions as the universal timing for all state changes
- "Ask a Question" persistent chatbot bar anchored to the viewport bottom

## 2. Color Palette & Roles

### Primary
- **Electric Blue** (`#3E6AE1`): The ONLY chromatic color in the system. Used strictly for: primary CTA button background ("Order Now"), primary CTA focus/active border ring, and text-link color. Never as a background tint, glow, decorative fill, atmospheric wash, or heading accent. If it shows up anywhere other than a button or a link, it's wrong.
- **Pure White** (`#FFFFFF`): Primary canvas — backgrounds, panels, navigation, surface tiles. The substrate that lets photography breathe.
- **Pure Black** (`#000000`) / **Carbon Dark** (`#171A20`): Secondary canvas — Tesla alternates full black sections with full white sections. On black sections, all text inverts to white and the same minimalism rules apply (no gradients, no glows, no atmospheric color).

### Secondary & Accent
- No additional accent colors exist. Promotional eyebrow text over hero imagery is plain white, not blue — the only blue on a hero is the CTA itself
- Tesla deliberately avoids color variety to maintain extreme visual discipline
...
### Decorative Depth
- No gradients, glows, or atmospheric effects on UI elements — ever
- The hero imagery itself provides all visual richness; UI chrome stays flat and neutral around it
- The only permissible non-solid background is a very faint bottom-edge darken (to ~35% black over the lower third of a hero image) strictly when white overlay text would otherwise fail contrast — not for mood, not for atmosphere
